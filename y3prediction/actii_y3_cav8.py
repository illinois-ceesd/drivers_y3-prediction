...
class InitACTII_Ramp:
    r"""Solution initializer for flow in the ACT-II facility

    This initializer starts with a shock upstream of the nozzle through
    ...
    def __call__(self, dcoll, x_vec, eos, *, time=0.0):
        ...
        area_ratio = inlet_height/throat_height
        mach = getMachFromAreaRatio(area_ratio=area_ratio,
                                    gamma=gamma,
                                    mach_guess=0.01)
        ...
        gamma_error = (self._gamma_guess - gamma)
        gamma_iter = gamma
        toler = 1.e-6
        ...
        while gamma_error > toler:
            ...
            cv = make_conserved(dim=self._dim, mass=mass, momentum=mom,
                                energy=energy, species_mass=mass*y)
            gamma = eos.gamma(cv, temp_left)
            gamma_error = (gamma_iter - gamma)
            gamma_iter = gamma
