...
Printf("boundratiocorner = %f", boundratiocorner);
...
Field[3002].DistMin = 0.002;
Field[3002].DistMax = 5;
Field[3002].StopAtDistMax = 1;

//Create threshold field that varrries element size near boundaries
// this is for the nozzle expasion only
cornerfac = boundrationozzle*boundratiocorner;
Field[3003] = Threshold;
Field[3003].InField = 3001;
Field[3003].SizeMin = 1.5*nozzlesize/cornerfac;
Field[3003].SizeMax = 1.5*nozzlesize/cornerfac*(2.-1./cornerfac);
Field[3003].DistMin = 0.002;
Field[3003].DistMax = 5;
Field[3003].StopAtDistMax = 1;

//Create threshold field that varrries element size near boundaries
// this is for the nozzle throat only
cornerfac = boundrationozzle*boundratiocorner;
Field[3004] = Threshold;
Field[3004].InField = 3001;
Field[3004].SizeMin = nozzlesize/cornerfac;
Field[3004].SizeMax = nozzlesize/cornerfac*(2.-1./cornerfac);
Field[3004].DistMin = 0.002;
Field[3004].DistMax = 5;
Field[3004].StopAtDistMax = 1;

// restrict the nozzle bl meshing to the nozzle only
Field[2020] = MathEval;
Field[2020].F = Sprintf("F3002 + (F3004 - F3002)*(0.5*(1.0 - tanh(%g*(x - %g))))*(0.5*(1.0 - tanh(%g*(%g - x))))", sigma, nozzle_end, sigma, nozzle_start);

// restrict the nozzle expansion bl meshing to the nozzle expansion only
Field[2021] = MathEval;
Field[2021].F = Sprintf("F3002 + (F3003 - F3002)*(0.5*(1.0 - tanh(%g*(x - %g))))*(0.5*(1.0 - tanh(%g*(%g - x))))", sigma, nozzle_exp_end, sigma, nozzle_end);

// Create distance field from corners for wall meshing, excludes cavity, injector
Field[3011] = Distance;
Field[3011].CurvesList = {
    18, 23, 22, 20, 21, // bottom aft
    35, 40, 39, 37, 38, // bottom fore
    77, 76, 71, 62, 69, 68                      // cavity spanwise edges
};
Field[3001].Sampling = 1000;
////
//Create threshold field that varrries element size near boundaries
cornerfac = boundratiocavity*boundratiocorner;
Field[3012] = Threshold;
Field[3012].InField = 3011;
Field[3012].SizeMin = cavitysize/cornerfac;
Field[3012].SizeMax = cavitysize/cornerfac*(2.-1./cornerfac);
Field[3012].DistMin = 0.002;
Field[3012].DistMax = 5;
Field[3012].StopAtDistMax = 1;
...
Field[100] = Min;
Field[100].FieldsList = {2010, 2011, 2020, 2021, 3012, 3, 4, 5, 9, 12, 16, 18, 20, 21, 22, 23, 102, 105, 118};
