    ...
    executor = HighThroughputExecutor(
        ...
        provider=LSFProvider(launcher=JsrunLauncher(overrides='-g 1 -a 1'),
            ...
            worker_init=(
                ...
                'export POCL_CACHE_ROOT="/tmp/$USER/xdg-scratch"\n'),
            project='uiuc')
        )
...
def build_execution_string(module="driver.py", yml="run_params.yaml",
    ...
    cmd = f"python -u -O -m mpi4py {module} -i {yml}"
    if lazy:
        cmd += " --lazy"
    if not log:
        cmd += " --nolog"
...
def main():
    ...
    run_mirge = execute(execution_string=conda_str+mirge_cmd,
    ...
    for app_future in monitor_restart_data:
        ...
        make_viz_data.append(
            execute(execution_string=conda_str+mirge_viz_cmd,
